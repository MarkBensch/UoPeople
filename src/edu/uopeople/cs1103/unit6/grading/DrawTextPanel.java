package edu.uopeople.cs1103.unit6.grading;
...
public class DrawTextPanel extends JPanel  {
	...
	// named myString.  (If it's null, nothing is shown.  This
	...
	private ArrayList<DrawTextItem> myString = new ArrayList<DrawTextItem>();
	...
	private class Canvas extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			if (myString != null){
				Iterator<DrawTextItem> iter = myString.iterator();
				while(iter.hasNext()){
...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
		...
		myString.add(s);  // add to the ArrayList
	...
	private void doMenuCommand(String command) {
		 ...
		 */
			...
			myString.clear();   // clear the list
			undoMenuItem.setEnabled(false);
			canvas.repaint();
		}
		else if (command.equals("Remove Item")) { // remove the most recently added string
			DrawTextItem temp = myString.get(myString.size()-1);//get the last item
			myString.remove(temp);   // remove the last item
			if(myString.size() == 0)
				undoMenuItem.setEnabled(false);
			canvas.repaint();
		}
