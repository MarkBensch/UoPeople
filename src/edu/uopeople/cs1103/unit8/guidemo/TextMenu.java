package edu.uopeople.cs1103.unit8.guidemo;
...
public class TextMenu extends JMenu {
	...
	private JCheckBoxMenuItem left = new JCheckBoxMenuItem("Left");
	private JCheckBoxMenuItem right = new JCheckBoxMenuItem("Right");
	private JCheckBoxMenuItem center = new JCheckBoxMenuItem("Center");
	...
	public TextMenu(DrawPanel owner) {
		...
		final JMenuItem spacing = new JMenuItem("Line Spacing");
		spacing.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				double currentSize = panel.getTextItem().getLineHeightMultiplier();
				String s = JOptionPane.showInputDialog(panel, "What is the Line spacing you would like?", currentSize);
				if (s != null && s.trim().length() > 0) {
					try {
						...
						JOptionPane.showMessageDialog(panel, s + " is not a legal spacing.\n"
	...
	/**
	 * create a submenu to toggle justification of text
	 * @return JMenu of text alignment JCheckboxMenuItems
	 * @author Mark bensch
	 */
	public JMenu createJustifyMenu(){

		left = new JCheckBoxMenuItem("Left");
		right = new JCheckBoxMenuItem("Right");
		center = new JCheckBoxMenuItem("Center");
		ActionListener al = new ActionListener(){
			public void actionPerformed(ActionEvent evt) {
				if(left.isSelected() && panel.getTextItem().getJustify() != TextItem.LEFT){
					panel.getTextItem().setJustify(TextItem.LEFT);
					right.setState( false );
					center.setState( false );
				}
				if(right.isSelected() && panel.getTextItem().getJustify() != TextItem.RIGHT){
					panel.getTextItem().setJustify(TextItem.RIGHT);
					left.setState( false );
					center.setState( false );
				}
				if(center.isSelected() && panel.getTextItem().getJustify() != TextItem.CENTER){
		...
		JMenu textAlign = new JMenu( "Justify Text" );
		textAlign.add(left);
		textAlign.add(right);
